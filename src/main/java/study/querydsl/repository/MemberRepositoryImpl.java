...
//    }
    ...
    public Page<MemberTeamDto> searchPageCountQuery(MemberSearchCondition condition, Pageable pageable) {
        ...
        /**
         * offset이 1이고 limit가 100일때 실제 데이터가 3밖에 없다면 100을 넘지 않는다면 굳이 total 쿼리를 부를 필요가 없음
         * 그럴 경우 사용하면 전체 데이터의 갯수를 가져오면 되는거다.
         */
    ...
    public List<MemberTeamDto> searchPageSimple(MemberSearchCondition condition, Pageable pageable) {
        JPQLQuery<MemberTeamDto> jpqlQuery = from(member).leftJoin(member.team, team)
                .where(
                        usernameEq(condition.getUsername()),
                        teamNameEq(condition.getTeamName()),
                        ageGoe(condition.getAgeGoe()),
        ...
        List<MemberTeamDto> fetch = result.fetch();
